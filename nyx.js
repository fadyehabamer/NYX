/*!
 * Nyx — runtime · v1.0.2 · MIT License
...
(function (root, factory) {
  ...
  var _uid = 0;                                                 // unique-id seed for aria wiring
  ...
  function clearInert() { $$('[data-nyx-inert]').forEach(function (n) { n.removeAttribute('inert'); n.removeAttribute('data-nyx-inert'); }); }
  function dialogSemantics(m) {                                 // role=dialog on the actual panel (modal-box for modals; the element itself for drawer/sheet)
    ...
    if (!dlg.getAttribute('aria-label') && !dlg.getAttribute('aria-labelledby')) {
      var h = dlg.querySelector('.nyx-modal-title, .nyx-drawer-title, .nyx-sheet-title, [data-nyx-dialog-title], h1, h2, h3');
  ...
  function togglePopover(node, forceState) {
    ...
    var trg = p.querySelector('[data-nyx-toggle="popover"]');   // keep the trigger's state in sync on every path
    if (trg) trg.setAttribute('aria-expanded', willOpen ? 'true' : 'false');
  }

  /* ---------- dropdown ---------- */
  function closeDropdowns(except) {
    $$('.nyx-dropdown.open').forEach(function (d) {
      if (d !== except) { d.classList.remove('open'); var tg = d.querySelector('[data-nyx-toggle="dropdown"]'); if (tg) tg.setAttribute('aria-expanded', 'false'); }
    });
  }
  ...
  function openCommandPalette() {
    ...
    var inp = cp.querySelector('input');
    if (inp) { inp.value = ''; inp.dispatchEvent(new Event('input', { bubbles: true })); setTimeout(function () { inp.focus(); }, 60); }
  ...
  function initPalette(root) {
    $$('.nyx-command-palette', root).filter(function (cp) { return !cp._nyxCp; }).forEach(function (cp) {
      cp._nyxCp = true;
      ...
      var input = cp.querySelector('.nyx-cp-input input'), list = cp.querySelector('.nyx-cp-list');
      if (!input || !list) return;
      var items = $$('.nyx-cp-item', list);
      ...
      items.forEach(function (it) { it.setAttribute('role', 'option'); it.setAttribute('aria-selected', 'false'); if (!it.id) it.id = 'nyx-cp-opt-' + (++_uid); });
      function visible() { return items.filter(function (it) { return it.style.display !== 'none'; }); }
      function setActive(it) {
        items.forEach(function (o) { o.classList.remove('active'); o.setAttribute('aria-selected', 'false'); });
      ...
      input.addEventListener('keydown', function (e) {
        ...
        var cur = vis.indexOf(cp.querySelector('.nyx-cp-item.active'));
        ...
        else if (e.key === 'Enter') { var a = cp.querySelector('.nyx-cp-item.active'); if (a) { e.preventDefault(); a.click(); } }
      });
    });
  }
  ...
  function toast(message, typeOrOpts, ms) {
    ...
    var urgent = type === 'danger' || type === 'warning';      // errors interrupt; the rest are polite
  ...
  function toggleCollapse(trigger) {
    ...
    if (acc && willOpen && !acc.hasAttribute('data-multi')) {     // data-multi keeps siblings open
      ...
      $$('[data-nyx-toggle="collapse"].active', acc).forEach(function (h) { h.classList.remove('active'); h.setAttribute('aria-expanded', 'false'); });
    }
  ...
  function initDisclosure(root) {
    $$('[data-nyx-toggle="collapse"]', root).filter(function (t) { return !t._nyxDisc; }).forEach(function (t) {
      t._nyxDisc = true;
      var tg = el(t.getAttribute('data-nyx-target'));
  ...
  function initTriggers(root) {
    $$('[data-nyx-toggle="popover"]', root).filter(function (t) { return !t._nyxTp; }).forEach(function (t) { t._nyxTp = true; t.setAttribute('aria-haspopup', 'dialog'); if (!t.hasAttribute('aria-expanded')) t.setAttribute('aria-expanded', 'false'); });
    $$('[data-nyx-toggle="dropdown"]', root).filter(function (t) { return !t._nyxTd; }).forEach(function (t) { t._nyxTd = true; t.setAttribute('aria-haspopup', 'menu'); if (!t.hasAttribute('aria-expanded')) t.setAttribute('aria-expanded', 'false'); });
    $$('.nyx-fab-btn', root).filter(function (t) { return !t._nyxTf; }).forEach(function (t) { t._nyxTf = true; t.setAttribute('aria-haspopup', 'menu'); if (!t.hasAttribute('aria-expanded')) t.setAttribute('aria-expanded', 'false'); });
  }
  ...
  function initCarousel(root) {
    $$('.nyx-carousel', root).filter(function (c) { return !c._nyxCar; }).forEach(function (car) {
      ...
      car.addEventListener('keydown', function (e) {
        var rtl = docEl.getAttribute('dir') === 'rtl';            // arrows follow reading direction
  ...
  doc.addEventListener('click', function (e) {
    ...
    if (fab) { var f = fab.closest('.nyx-fab'); if (f) { fab.setAttribute('aria-expanded', f.classList.toggle('open') ? 'true' : 'false'); } return; }
  ...
  function initCombobox(root) {
    $$('.nyx-combobox', root).filter(function (c) { return !c._nyxCb; }).forEach(function (c) {
      ...
      opts.forEach(function (o) { o.setAttribute('role', 'option'); o.setAttribute('tabindex', '-1'); if (!o.id) o.id = 'nyx-cbo-' + (++_uid); });
  ...
  function initCharts(root) {
    $$('.nyx-chart-bars', root).filter(function (c) { return !c._nyxCh; }).forEach(function (c) {
      c._nyxCh = true;
      var bars = $$('.nyx-bar', c);
      bars.forEach(function (b) { if (!b.hasAttribute('tabindex')) b.setAttribute('tabindex', '0'); });   // keyboard reveals the value tooltip
      if (!c.getAttribute('role')) c.setAttribute('role', 'img');
      if (!c.getAttribute('aria-label')) {
        var summary = c.getAttribute('data-nyx-label');
        if (!summary) {
          var pts = bars.map(function (b) { return ((b.getAttribute('data-label') || '') + ' ' + (b.getAttribute('data-val') || '')).trim(); }).filter(Boolean);
          summary = 'Bar chart' + (pts.length ? ': ' + pts.join(', ') : '');
        }
        c.setAttribute('aria-label', summary);
      }
    });
    $$('.nyx-chart-line, .nyx-chart-donut, .nyx-chart-pie, .nyx-donut', root).filter(function (c) { return !c._nyxCh; }).forEach(function (c) {
      c._nyxCh = true;
      if (!c.getAttribute('role')) c.setAttribute('role', 'img');
  ...
  function initMultiselect(root) {
    $$('.nyx-multiselect', root).filter(function (m) { return !m._nyxMs; }).forEach(function (m) {
      ...
      function chipFor(val) {                                    // attribute-match, not interpolated selector (injection-safe)
        var chips = $$('.nyx-chip', control), i;
      ...
      function removeVal(v) {
        ...
        var ch = chipFor(v); if (ch) ch.remove();
      }
      function toggle(o) {
  ...
  function initZakat(root) {
    $$('.nyx-zakat', root).filter(function (z) { return !z._nyxZ; }).forEach(function (z) {
      ...
      var nisab = parseFloat(z.getAttribute('data-nisab')) || 0;   // threshold below which no zakat is due
      ...
      function calc() {
        ...
        var below = nisab > 0 && v < nisab;                       // zakat is owed only at/above nisab
        z.classList.toggle('nyx-below-nisab', below);
        var due = below ? 0 : v * rate;
  ...
  function initQibla(root) {
    $$('.nyx-qibla[data-nyx-qibla], .nyx-qibla[data-coords]', root).filter(function (q) { return !q._nyxQ; }).forEach(function (q) {
      q._nyxQ = true;
      var deg, coords = q.getAttribute('data-coords');
      if (coords) { var p = coords.split(',').map(parseFloat); if (p.length === 2 && !isNaN(p[0]) && !isNaN(p[1])) deg = qiblaBearing(p[0], p[1]); }
      if (deg == null) deg = parseFloat(q.getAttribute('data-nyx-qibla')) || 0;
      deg = Math.round(deg * 10) / 10;
      var n = q.querySelector('.needle'); if (n) n.style.transform = 'rotate(' + deg + 'deg)';
  ...
  function initIdInput(root) {
    $$('.nyx-id-input', root).filter(function (w) { return !w._nyxId; }).forEach(function (w) {
      ...
      function check(commit) {
        ...
        var ok = /^[12]\d{9}$/.test(d) && luhnValid(d);           // format + checksum (not format alone)
        var bad = !ok && d.length > 0 && (commit || d.length >= 10);
  ...
  var HIJRI_MONTHS = ['محرّم', 'صفر', 'ربيع الأول', 'ربيع الآخر', 'جمادى الأولى', 'جمادى الآخرة', 'رجب', 'شعبان', 'رمضان', 'شوّال', 'ذو القعدة', 'ذو الحجّة'];
  var _HEPOCH = 1948439.5;
  function _gToJD(y, m, d) { var a = Math.floor((14 - m) / 12), yy = y + 4800 - a, mm = m + 12 * a - 3; return d + Math.floor((153 * mm + 2) / 5) + 365 * yy + Math.floor(yy / 4) - Math.floor(yy / 100) + Math.floor(yy / 400) - 32045; }
  function _jdToG(jd) { var a = jd + 32044, b = Math.floor((4 * a + 3) / 146097), c = a - Math.floor(146097 * b / 4), e2 = Math.floor((4 * c + 3) / 1461), e = c - Math.floor(1461 * e2 / 4), m = Math.floor((5 * e + 2) / 153); return { y: 100 * b + e2 - 4800 + Math.floor(m / 10), m: m + 3 - 12 * Math.floor(m / 10), d: e - Math.floor((153 * m + 2) / 5) + 1 }; }
  function _isToJD(y, m, d) { return d + Math.ceil(29.5 * (m - 1)) + (y - 1) * 354 + Math.floor((3 + 11 * y) / 30) + _HEPOCH - 1; }
  function _jdToIs(jd) { jd = Math.floor(jd) + 0.5; var y = Math.floor((30 * (jd - _HEPOCH) + 10646) / 10631); var m = Math.min(12, Math.ceil((jd - (29 + _isToJD(y, 1, 1))) / 29.5) + 1); return { y: y, m: m, d: (jd - _isToJD(y, m, 1)) + 1 }; }
  function _intlHijri(date) {
    try {
      var p = new Intl.DateTimeFormat('en-u-ca-islamic-umalqura-nu-latn', { day: 'numeric', month: 'numeric', year: 'numeric', timeZone: 'UTC' }).formatToParts(date);
      var o = {}; for (var i = 0; i < p.length; i++) if (p[i].type !== 'literal') o[p[i].type] = +p[i].value;
  ...
  function toHijri(y, m, d) {
    var date = y instanceof Date ? new Date(Date.UTC(y.getFullYear(), y.getMonth(), y.getDate())) : new Date(Date.UTC(y, m - 1, d));
    var h = _intlHijri(date) || _jdToIs(_gToJD(date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate()) + 0.5);
  ...
  function fromHijri(hy, hm, hd) {
    var s = _jdToG(Math.floor(_isToJD(hy, hm, hd))), dt = new Date(Date.UTC(s.y, s.m - 1, s.d));
    if (_intlHijri(dt)) {
      for (var i = 0; i < 4; i++) {
        var h = _intlHijri(dt);
  ...
  function formatHijri(input, opts) {
    var h = input instanceof Date ? toHijri(input) : input;
    var s = h.d + ' ' + (h.month || HIJRI_MONTHS[h.m - 1]) + ' ' + h.y + ' هـ';
  ...
  function initHijri(root) {
    ...
    $$('[data-nyx-hijri]', root).filter(function (w) { return !w._nyxHC; }).forEach(function (w) {
      w._nyxHC = true;
      var greg = w.querySelector('.nyx-hc-greg'), hd = w.querySelector('.nyx-hc-hd'), hm = w.querySelector('.nyx-hc-hm'), hy = w.querySelector('.nyx-hc-hy'), out = w.querySelector('[data-hijri-text]');
      if (hm && !hm.options.length) HIJRI_MONTHS.forEach(function (n, i) { var o = doc.createElement('option'); o.value = i + 1; o.textContent = (i + 1) + ' · ' + n; hm.appendChild(o); });
      function showOut(h) { if (out) out.textContent = formatHijri(h, { numerals: 'arab' }); }
      function fromGreg() {
        if (!greg || !greg.value) return;
        var p = greg.value.split('-'), h = toHijri(+p[0], +p[1], +p[2]);
        if (hd) hd.value = h.d; if (hm) hm.value = h.m; if (hy) hy.value = h.y;
        showOut(h);
      }
      function fromHij() {
        if (!hy || !hd || !hm) return;
        var y = +hy.value, m = +hm.value, d = +hd.value; if (!y || !m || !d) return;
        var dt = fromHijri(y, m, d);
      ...
      [hd, hm, hy].forEach(function (f) { if (f) f.addEventListener('change', fromHij); });
      if (greg && !greg.value) { var t = new Date(); greg.value = t.getFullYear() + '-' + _pad(t.getMonth() + 1) + '-' + _pad(t.getDate()); }
      fromGreg();
    });
  }

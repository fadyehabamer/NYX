...
((root, factory) => {
  ...
  (function applySaved() {
    const t = read('nyx-theme'); if (t) docEl.setAttribute('data-theme', t);
    const d = read('nyx-dir'); if (d) docEl.setAttribute('dir', d);
    const a = read('nyx-accent'); if (a && a !== 'violet') docEl.setAttribute('data-accent', a);
  })();
  ...
  function focusables(c) { return $$(FOCUSABLE, c).filter(e => e.offsetWidth > 0 || e.offsetHeight > 0 || e === doc.activeElement); }
  function currentOverlay() { return $('.nyx-modal.open') || $('.nyx-sheet.open') || $('.nyx-drawer.open') || $('.nyx-command-palette.open'); }
  function releaseFocus() { const l = _lastFocus; _lastFocus = null; if (l && l.focus) setTimeout(() => { try { l.focus(); } catch (e) {} }, 0); }
  ...
  function clearInert() { $$('[data-nyx-inert]').forEach(n => { n.removeAttribute('inert'); n.removeAttribute('data-nyx-inert'); }); }
  function dialogSemantics(m) {                                 // role=dialog on the actual panel (modal-box for modals; the element itself for drawer/sheet)
  ...
  function openModal(target) {
    ...
    const type = m.classList.contains('nyx-drawer') ? 'drawer' : (m.classList.contains('nyx-sheet') ? 'sheet' : 'modal');
  ...
  function close(target) {
    ...
    const type = t.classList.contains('nyx-drawer') ? 'drawer' : (t.classList.contains('nyx-sheet') ? 'sheet' : 'modal');
    t.dispatchEvent(new CustomEvent('nyx:' + type + '-hide', { bubbles: true }));
    if (!currentOverlay()) { if (_backdrop) _backdrop.classList.remove('open'); lockScroll(false); clearInert(); releaseFocus(); }
  }
  function closeAll() {
    $$('.nyx-modal.open, .nyx-drawer.open, .nyx-sheet.open').forEach(n => {
      n.classList.remove('open');
      const type = n.classList.contains('nyx-drawer') ? 'drawer' : (n.classList.contains('nyx-sheet') ? 'sheet' : 'modal');
      n.dispatchEvent(new CustomEvent('nyx:' + type + '-hide', { bubbles: true }));
    });
    if (_backdrop) _backdrop.classList.remove('open');
    closeCommandPalette();
    lockScroll(false);
    if (!currentOverlay()) { clearInert(); releaseFocus(); }
  }

  /* ---------- popover ---------- */
  function togglePopover(node, forceState) {
    let p = el(node); if (!p) return;
    if (!p.classList.contains('nyx-popover')) p = p.closest('.nyx-popover');
    if (!p) return;
    const willOpen = forceState !== undefined ? forceState : !p.classList.contains('open');
    if (willOpen) {
    ...
    const trg = p.querySelector('[data-nyx-toggle="popover"]');   // keep the trigger's state in sync on every path
  ...
  function closeDropdowns(except) {
    $$('.nyx-dropdown.open').forEach(d => {
      if (d !== except) { d.classList.remove('open'); const tg = d.querySelector('[data-nyx-toggle="dropdown"]'); if (tg) tg.setAttribute('aria-expanded', 'false'); }
    });
  }
  ...
  function closeCommandPalette() {
    const cp = paletteEl(); if (cp && cp.classList.contains('open')) {
      cp.classList.remove('open');
      cp.dispatchEvent(new CustomEvent('nyx:palette-hide', { bubbles: true }));
    }
    if (!currentOverlay()) { lockScroll(false); clearInert(); releaseFocus(); }
  }
  /* wire the command palette: live filtering + combobox keyboard nav + ARIA */
  function initPalette(root) {
    $$('.nyx-command-palette', root).filter(cp => !cp._nyxCp).forEach(cp => {
      cp._nyxCp = true;
      if (!cp.getAttribute('role')) { cp.setAttribute('role', 'dialog'); cp.setAttribute('aria-modal', 'true'); }
      ...
      items.forEach(it => { it.setAttribute('role', 'option'); it.setAttribute('aria-selected', 'false'); if (!it.id) it.id = 'nyx-cp-opt-' + (++_uid); });
      function visible() { return items.filter(it => it.style.display !== 'none'); }
      function setActive(it) {
        items.forEach(o => { o.classList.remove('active'); o.setAttribute('aria-selected', 'false'); });
        if (it) { it.classList.add('active'); it.setAttribute('aria-selected', 'true'); input.setAttribute('aria-activedescendant', it.id); it.scrollIntoView({ block: 'nearest' }); }
      ...
      input.addEventListener('keydown', e => {
        ...
        if (e.key === 'ArrowDown') { e.preventDefault(); setActive(vis[(cur + 1) % vis.length]); }
        else if (e.key === 'ArrowUp') { e.preventDefault(); setActive(vis[(cur - 1 + vis.length) % vis.length]); }
        else if (e.key === 'Home') { e.preventDefault(); setActive(vis[0]); }
        else if (e.key === 'End') { e.preventDefault(); setActive(vis[vis.length - 1]); }
        else if (e.key === 'Enter') { const a = cp.querySelector('.nyx-cp-item.active'); if (a) { e.preventDefault(); a.click(); } }
      });
    });
  }
  ...
  function toast(message, typeOrOpts, ms) {
    ...
    const urgent = type === 'danger' || type === 'warning';      // errors interrupt; the rest are polite
    t.setAttribute('role', urgent ? 'alert' : 'status');
    t.setAttribute('aria-live', urgent ? 'assertive' : 'polite');
    const icon = doc.createElement('span'); icon.className = 'nyx-toast-icon'; icon.setAttribute('aria-hidden', 'true'); icon.textContent = o.icon || ICONS[type] || '•';
    t.appendChild(icon);
    const body = doc.createElement('div'); body.className = 'nyx-toast-body';
    if (o.title) { const ti = doc.createElement('strong'); ti.className = 'nyx-toast-title'; ti.textContent = o.title; body.appendChild(ti); }
    const txt = doc.createElement('span'); txt.textContent = message; body.appendChild(txt);
    t.appendChild(body);
    let timer;
    function dismiss() { clearTimeout(timer); t.classList.add('nyx-out'); t.addEventListener('animationend', () => { t.remove(); }); }
    if (o.action) {                                            // action button (e.g. Undo)
      const b = doc.createElement('button'); b.type = 'button'; b.className = 'nyx-toast-action';
      b.textContent = o.action.label || o.action;
      b.addEventListener('click', () => { if (o.action.onClick) o.action.onClick(); dismiss(); });
      t.appendChild(b);
    }
    if (o.dismissible) {                                       // ✕ close button
      const x = doc.createElement('button'); x.type = 'button'; x.className = 'nyx-toast-close'; x.setAttribute('aria-label', 'Dismiss'); x.textContent = '✕';
  ...
  function snackbar(message, opts) {
    opts = opts || {};
    const s = doc.createElement('div'); s.className = 'nyx-snackbar'; s.setAttribute('role', 'status');
    const msg = doc.createElement('span'); msg.className = 'nyx-snackbar-msg'; msg.textContent = message; s.appendChild(msg);
    let timer;
    function dismiss() { clearTimeout(timer); s.classList.add('nyx-out'); s.addEventListener('animationend', () => { s.remove(); }); }
    if (opts.action) {
      const b = doc.createElement('button'); b.type = 'button'; b.className = 'nyx-snackbar-action'; b.textContent = opts.action;
      b.addEventListener('click', () => { if (opts.onAction) opts.onAction(); dismiss(); });
      s.appendChild(b);
    }
  ...
  function htmlEsc(s) { const d = doc.createElement('div'); d.textContent = s == null ? '' : s; return d.innerHTML; }
  function confirmDialog(message, opts) {
    opts = opts || {};
    return new Promise(resolve => {
      const modal = doc.createElement('div'); modal.className = 'nyx-modal open';
      ...
      const bd = backdrop(); bd.classList.add('open'); lockScroll(true);
      function done(val) {
      ...
      modal.addEventListener('click', e => {
        ...
        const a = e.target.closest('[data-act]'); if (a) done(a.getAttribute('data-act') === 'ok');
      });
      doc.addEventListener('keydown', onKey);
      const f = modal.querySelector('[data-act="ok"]'); if (f) setTimeout(() => { f.focus(); }, 60);
    });
  }

  /* ---------- tabs / pills ---------- */
  function activateTab(btn) {
    const group = btn.closest('[data-nyx-tabs]'); if (!group) return;
  ...
  function toggleCollapse(trigger) {
    const t = el(trigger.getAttribute('data-nyx-target')); if (!t) return;
    ...
    if (acc && willOpen && !acc.hasAttribute('data-multi')) {     // data-multi keeps siblings open
      $$('.nyx-collapse.open', acc).forEach(c => {
        c.classList.remove('open');
        c.dispatchEvent(new CustomEvent('nyx:collapse-hide', { bubbles: true }));
      });
      $$('[data-nyx-toggle="collapse"].active', acc).forEach(h => { h.classList.remove('active'); h.setAttribute('aria-expanded', 'false'); });
    }
  ...
  function initAccordion(root) {
    $$('[data-nyx-accordion][data-open]', root).filter(a => !a._nyxAcc).forEach(acc => {
      ...
      acc.getAttribute('data-open').split(',').forEach(n => {
        const trig = triggers[parseInt(n, 10)]; if (!trig) return;
        const t = el(trig.getAttribute('data-nyx-target')); if (!t) return;
        t.classList.add('open'); trig.classList.add('active');
      });
    });
  }
  /* decorate disclosure (collapse/accordion) triggers + their regions with ARIA */
  ...
  /* declare popup affordances on overlay triggers so AT announces them */
  function initTriggers(root) {
    $$('[data-nyx-toggle="popover"]', root).filter(t => !t._nyxTp).forEach(t => { t._nyxTp = true; t.setAttribute('aria-haspopup', 'dialog'); if (!t.hasAttribute('aria-expanded')) t.setAttribute('aria-expanded', 'false'); });
    $$('[data-nyx-toggle="dropdown"]', root).filter(t => !t._nyxTd).forEach(t => { t._nyxTd = true; t.setAttribute('aria-haspopup', 'menu'); if (!t.hasAttribute('aria-expanded')) t.setAttribute('aria-expanded', 'false'); });
    $$('.nyx-fab-btn', root).filter(t => !t._nyxTf).forEach(t => { t._nyxTf = true; t.setAttribute('aria-haspopup', 'menu'); if (!t.hasAttribute('aria-expanded')) t.setAttribute('aria-expanded', 'false'); });
  }

  /* ---------- carousel ---------- */
  function carouselStep(car, dir) {
    const slides = $$('.nyx-slide', car); if (!slides.length) return;
  ...
  function carouselSet(car, i, slides) {
    ...
    $$('.nyx-carousel-dots button', car).forEach((d, n) => { d.classList.toggle('active', n === i); });
    if (oldActive !== i) {
  ...
  function initCarousel(root) {
    $$('.nyx-carousel', root).filter(c => !c._nyxCar).forEach(car => {
      ...
      car.addEventListener('keydown', e => {
        const rtl = docEl.getAttribute('dir') === 'rtl';            // arrows follow reading direction
        if (e.key === 'ArrowLeft') { carouselStep(car, rtl ? 'next' : 'prev'); e.preventDefault(); }
        else if (e.key === 'ArrowRight') { carouselStep(car, rtl ? 'prev' : 'next'); e.preventDefault(); }
      });
      let x0 = null;                                              // pointer swipe
  ...
  function stepperAdjust(btn) {
    const wrap = btn.closest('.nyx-stepper'), inp = wrap && wrap.querySelector('input'); if (!inp) return;
  ...
  function initSortable(table) {
    ...
    ths.forEach((th, col) => {
      let asc = true;
      th.setAttribute('tabindex', '0'); th.setAttribute('role', 'columnheader'); th.setAttribute('aria-sort', 'none');
      function doSort() {
        ...
        ths.forEach(o => { if (o !== th) o.setAttribute('aria-sort', 'none'); });
  ...
  doc.addEventListener('click', e => {
    ...
    if (toggle) {
      ...
      if (kind === 'dropdown') {
        e.preventDefault();
        const dd = toggle.closest('.nyx-dropdown'), willOpen = dd && !dd.classList.contains('open');
        closeDropdowns(dd);
        if (dd) {
        ...
        if (willOpen) { const fi = dd.querySelector('.nyx-dropdown-item'); if (fi) setTimeout(() => { fi.focus(); }, 20); }
        return;
      }
    }
    ...
    if (slide) { const car = slide.closest('.nyx-carousel'); if (car) carouselStep(car, slide.getAttribute('data-nyx-slide')); return; }
    const dot = e.target.closest('[data-nyx-slide-to]');
    if (dot) { const c2 = dot.closest('.nyx-carousel'); if (c2) carouselSet(c2, +dot.getAttribute('data-nyx-slide-to')); return; }

    const cpItem = e.target.closest('.nyx-command-palette [data-nyx-target]');
    if (cpItem) { closeAll(); const sec = el(cpItem.getAttribute('data-nyx-target')); if (sec) sec.scrollIntoView(); return; }

    const hnode = e.target.closest('.nyx-hierarchy-node.has-kids');
    if (hnode) { const hli = hnode.closest('li'); if (hli) hli.classList.toggle('nyx-collapsed'); return; }
    ...
    if (fab) { const f = fab.closest('.nyx-fab'); if (f) { fab.setAttribute('aria-expanded', f.classList.toggle('open') ? 'true' : 'false'); } return; }
    ...
    if (!e.target.closest('.nyx-popover')) $$('.nyx-popover.open').forEach(o => { togglePopover(o, false); });
    if (!e.target.closest('.nyx-dropdown')) closeDropdowns();
    if (!e.target.closest('.nyx-combobox')) $$('.nyx-combobox.open').forEach(o => { o.classList.remove('open'); });
    if (!e.target.closest('.nyx-multiselect')) $$('.nyx-multiselect.open').forEach(o => { o.classList.remove('open'); });
    if (!e.target.closest('.nyx-datepicker')) $$('.nyx-datepicker.open').forEach(o => { o.classList.remove('open'); });
    if (!e.target.closest('.nyx-fab')) $$('.nyx-fab.open').forEach(o => { o.classList.remove('open'); });
    if (!e.target.closest('.nyx-context-menu')) $$('.nyx-context-menu.open').forEach(o => { o.classList.remove('open'); });
  });
  ...
  doc.addEventListener('keydown', e => {
    ...
    if (tabEl && (e.key === 'ArrowRight' || e.key === 'ArrowLeft' || e.key === 'Home' || e.key === 'End')) {
      const grp = tabEl.closest('[data-nyx-tabs]'); if (grp) {
        e.preventDefault();
    ...
    if (menu && (e.key === 'ArrowDown' || e.key === 'ArrowUp')) {
      const items = $$('.nyx-dropdown-item', menu); if (items.length) {
        e.preventDefault();
    ...
    if (e.key === 'Escape') {
      ...
      $$('.nyx-popover.open').forEach(o => { togglePopover(o, false); });
      $$('.nyx-combobox.open, .nyx-multiselect.open, .nyx-datepicker.open, .nyx-fab.open, .nyx-context-menu.open').forEach(o => { o.classList.remove('open'); });
    }
  });
  ...
  doc.addEventListener('input', e => {
    ...
    if (otp && e.target.tagName === 'INPUT') {
      ...
      if (e.target.value) { const ins = $$('input', otp), i = ins.indexOf(e.target); if (ins[i + 1]) ins[i + 1].focus(); }
    }
  });
  doc.addEventListener('keydown', e => {
    const otp = e.target.closest('.nyx-otp');
    if (otp && e.key === 'Backspace' && !e.target.value) { const ins = $$('input', otp), i = ins.indexOf(e.target); if (ins[i - 1]) ins[i - 1].focus(); }
    ...
    if (ti && e.target.tagName === 'INPUT' && e.key === 'Enter') {
      ...
      const chip = doc.createElement('span'); chip.className = 'nyx-chip';
      chip.appendChild(doc.createTextNode(val + ' '));
      const x = doc.createElement('span'); x.className = 'nyx-chip-x'; x.setAttribute('role', 'button'); x.setAttribute('aria-label', 'remove'); x.textContent = '×';
  ...
  function sliderFill(s) {
    ...
    let v = parseFloat(s.value);if (isNaN(v)) v = min;
    s.style.setProperty('--nyx-slider', max > min ? ((v - min) / (max - min)) * 100 : 0);
    const sel = s.getAttribute('data-output');                   // live value into any element
    if (sel) { const o = el(sel); if (o) o.textContent = (s.getAttribute('data-prefix') || '') + v + (s.getAttribute('data-suffix') || ''); }
  }
  ...
  function wantsArab(node) {
    for (let n = node; n && n.getAttribute; n = n.parentElement) {
      let v = n.getAttribute('data-nyx-numerals'); if (v == null) v = n.getAttribute('data-numerals');
  ...
  function initNumerals(root) {
    $$('[data-nyx-numerals="arab"]', root).filter(e => !e._nyxNum).forEach(e => {
      ...
      nodes.forEach(n => { if (/[0-9]/.test(n.nodeValue)) n.nodeValue = toArabicNumerals(n.nodeValue); });
    });
  }
  ...
  function initCombobox(root) {
    $$('.nyx-combobox', root).filter(c => !c._nyxCb).forEach(c => {
      ...
      function setOpen(on) { c.classList.toggle('open', on); input.setAttribute('aria-expanded', on ? 'true' : 'false'); if (!on) input.removeAttribute('aria-activedescendant'); }
      function filter() {
        const q = (input.value || '').trim().toLowerCase();
        let shown = 0;
        opts.forEach(o => { const hit = o.textContent.toLowerCase().includes(q); o.hidden = !hit; if (hit) shown++; });
        c.classList.toggle('empty', shown === 0);
      }
      function vis() { return opts.filter(o => !o.hidden); }
      function setActive(list, i) {
        opts.forEach(o => { o.classList.remove('active'); o.setAttribute('aria-selected', 'false'); });
        if (list[i]) { list[i].classList.add('active'); list[i].setAttribute('aria-selected', 'true'); input.setAttribute('aria-activedescendant', list[i].id); list[i].scrollIntoView({ block: 'nearest' }); }
        else input.removeAttribute('aria-activedescendant');
      }
      ...
      input.addEventListener('focus', () => { setOpen(true); filter(); });
      ...
      input.addEventListener('keydown', e => {
        ...
        else if (e.key === 'Enter') { const a = c.querySelector('.nyx-combobox-opt.active') || list[0]; if (a) { e.preventDefault(); choose(a); } }
        else if (e.key === 'Escape') { setOpen(false); }
      });
      c.addEventListener('mousedown', e => {
        const o = e.target.closest('.nyx-combobox-opt'); if (!o) return; e.preventDefault(); choose(o);
      });
    });
  }

  /* ---------- charts: image semantics + keyboard-revealable values ---------- */
  function initCharts(root) {
    $$('.nyx-chart-bars', root).filter(c => !c._nyxCh).forEach(c => {
      c._nyxCh = true;
      const bars = $$('.nyx-bar', c);
      bars.forEach(b => { if (!b.hasAttribute('tabindex')) b.setAttribute('tabindex', '0'); });   // keyboard reveals the value tooltip
  ...
  function initMultiselect(root) {
    $$('.nyx-multiselect', root).filter(m => !m._nyxMs).forEach(m => {
      ...
      opts.forEach(o => { o.setAttribute('role', 'option'); o.setAttribute('aria-selected', 'false'); });
      function valOf(o) { return o.getAttribute('data-value') || o.textContent.trim(); }
      function chipFor(val) {
        // attribute-match, not interpolated selector (injection-safe)
      ...
      function addChip(val, label) {
        ...
        chip.appendChild(doc.createTextNode(label + ' '));       // textContent, not innerHTML (XSS-safe)
        const x = doc.createElement('span');
        x.className = 'nyx-chip-x'; x.setAttribute('role', 'button'); x.setAttribute('aria-label', 'remove'); x.textContent = '×';
        chip.appendChild(x);
        control.insertBefore(chip, input);
      }
      function removeVal(v) {
        opts.forEach(o => { if (valOf(o) === v) { o.classList.remove('selected'); o.setAttribute('aria-selected', 'false'); } });
        const ch = chipFor(v); if (ch) ch.remove();
      }
      function toggle(o) {
        const on = o.classList.toggle('selected'); o.setAttribute('aria-selected', on ? 'true' : 'false');
      ...
      if (menu) menu.addEventListener('mousedown', e => {
        const o = e.target.closest('.nyx-multiselect-opt'); if (!o) return; e.preventDefault(); toggle(o);
      });
      if (input) input.addEventListener('keydown', e => {
        ...
        else if (e.key === 'ArrowDown') { e.preventDefault(); m.classList.add('open'); const f = menu && menu.querySelector('.nyx-multiselect-opt'); if (f) f.scrollIntoView({ block: 'nearest' }); }
      });
    });
  }
  ...
  function calMonthHtml(y, m, sel, today, cfg) {
    ...
    let first = new Date(y, m, 1).getDay(); if (ws) first = (first + 6) % 7;     // shift Monday to col 0
    ...
    let mOpts = ''; MONTHS.forEach((name, i) => { mOpts += '<option value="' + i + '"' + (i === m ? ' selected' : '') + '>' + name + '</option>'; });
    let yOpts = ''; for (let yy = hi; yy >= lo; yy--) yOpts += '<option value="' + yy + '"' + (yy === y ? ' selected' : '') + '>' + yy + '</option>';
    const dow = ws ? DOW.slice(1).concat(DOW.slice(0, 1)) : DOW;
    let h = '<div class="nyx-calendar-head">'
    ...
    dow.forEach(d => { h += '<span class="dow">' + d + '</span>'; });
  ...
  function initDatepicker(root) {
    $$('[data-nyx-datepicker]', root).filter(d => !d._nyxDp).forEach(dp => {
      ...
      const now = new Date(), st = { y: now.getFullYear(), m: now.getMonth(), sel: null, today: { y: now.getFullYear(), m: now.getMonth(), d: now.getDate() } };
      const fmt = dp.getAttribute('data-format') || 'YYYY-MM-DD';
      const cfg = calCfg(dp, true);                                 // datepicker shows the Today/Clear footer
      function render() { cal.innerHTML = calMonthHtml(st.y, st.m, st.sel, st.today, cfg); }
      render();
      input.addEventListener('focus', () => { dp.classList.add('open'); });
      cal.addEventListener('change', e => {               // month / year dropdowns
        const s = e.target.closest('[data-cal-set]'); if (!s) return;
        e.stopPropagation();
      ...
      cal.addEventListener('click', e => {
        ...
        const go = e.target.closest('[data-cal-go]');               // Today / Clear footer
        if (go) {
          e.stopPropagation();
  ...
  function initCalendar(root) {
    $$('.nyx-calendar[data-nyx-calendar]', root).filter(c => !c._nyxCal).forEach(cal => {
      ...
      const cfg = calCfg(cal, cal.hasAttribute('data-footer'));     // add data-footer for Today/Clear
      function render() { cal.innerHTML = calMonthHtml(st.y, st.m, st.sel, st.today, cfg); }
      render();
      cal.addEventListener('change', e => {               // month / year dropdowns
        const s = e.target.closest('[data-cal-set]'); if (!s) return;
  ...
  function hijriParts(d, loc) {
    ...
    const o = {}; f.formatToParts(d).forEach(p => { if (p.type !== 'literal') o[p.type] = parseInt(p.value, 10); });
    return o;                              // { day, month, year }
  }
  function hijriMonth(anchor, loc) {
    const hp = hijriParts(anchor, loc);
    const day1 = new Date(anchor);day1.setDate(day1.getDate() - (hp.day - 1));   // Gregorian date of Hijri day 1
    const days = [];
  ...
  function initHijriCalendar(cal) {
    ...
    cal.addEventListener('click', e => {
      ...
      const day = e.target.closest('.day[data-day]');
      if (day) {
        $$('.day.selected', cal).forEach(x => { x.classList.remove('selected'); });
  ...
  function rangeFill(wrap) {
    const ins = $$('input[type="range"]', wrap);if (ins.length < 2) return;
  ...
  function initRange(root) {
    $$('.nyx-range', root).filter(r => $$('input[type="range"]', r).length >= 2 && !r._nyxRg)
      .forEach(r => { r._nyxRg = true; rangeFill(r); });
  }
  doc.addEventListener('input', e => {
  ...
  function initKanban(root) {
    $$('.nyx-kanban', root).filter(k => !k._nyxKb).forEach(board => {
      board._nyxKb = true;
      let dragging = null;
      function mark() { $$('.nyx-kanban-card', board).forEach(c => { c.setAttribute('draggable', 'true'); }); }
      mark();
      ...
      board.addEventListener('dragstart', e => {
        const card = e.target.closest('.nyx-kanban-card'); if (!card) return;
      ...
      board.addEventListener('dragend', () => {
        ...
        $$('.nyx-kanban-col', board).forEach(col => { col.classList.remove('nyx-drop-over'); });
        if (dragging) dragging.dispatchEvent(new CustomEvent('nyx:kanban-move', { bubbles: true }));
        dragging = null;
      });
      board.addEventListener('dragover', e => {
        if (!dragging) return;
        const col = e.target.closest('.nyx-kanban-col'); if (!col) return;
        e.preventDefault(); e.dataTransfer.dropEffect = 'move';
        $$('.nyx-kanban-col', board).forEach(c => { c.classList.toggle('nyx-drop-over', c === col); });
        const after = cardAfter(col, e.clientY);
        if (after == null) col.appendChild(dragging);
        else col.insertBefore(dragging, after);
      });
      board.addEventListener('drop', e => { if (dragging) e.preventDefault(); });
    });
  }
  ...
  function lightboxEl() {
    ...
    function show(i) { const a = _lightbox._imgs; if (!a.length) return; _lightbox._i = (i + a.length) % a.length; img.src = a[_lightbox._i]; }
    ...
    _lightbox.querySelector('.nyx-lightbox-prev').addEventListener('click', e => { e.stopPropagation(); show(_lightbox._i - 1); });
    _lightbox.querySelector('.nyx-lightbox-next').addEventListener('click', e => { e.stopPropagation(); show(_lightbox._i + 1); });
    _lightbox.addEventListener('click', e => { if (e.target === _lightbox) hide(); });
    doc.addEventListener('keydown', e => {
  ...
  function initVideoFacade(root) {
    $$('.nyx-video[data-embed]', root).filter(v => !v._nyxVid).forEach(v => {
      ...
      v.addEventListener('click', () => {
        ...
        v.style.height = h + 'px';
        const poster = v.querySelector('img'); if (poster) poster.style.display = 'none';
  ...
  function syncBackTop() {
    ...
    $$('.nyx-to-top').forEach(b => { b.classList.toggle('show', sy > 320); });
    const sh = docEl.scrollHeight - docEl.clientHeight;
    const pct = sh > 0 ? (sy / sh) * 100 : 0;
    $$('.nyx-scroll-progress').forEach(p => { p.style.width = pct + '%'; });
  ...
  const progress = {
    ...
    done: function () { clearInterval(_tbTimer); const s = topSpan(); s.style.width = '100%'; setTimeout(() => { s.style.opacity = '0'; setTimeout(() => { s.style.width = '0'; }, 350); }, 220); return progress; }
  };

  /* ---------- countdown (Iftar / Suhoor / sale) ---------- */
  function tickCountdown(c) {
    const diff = Math.max(0, Math.floor((c._nyxTarget - new Date()) / 1000));
  ...
  function initCountdown(root) {
    $$('.nyx-countdown[data-nyx-countdown], .nyx-countdown[data-date]', root).filter(c => !c._nyxCd).forEach(c => {
      ...
      if (dateAttr) { target = new Date(dateAttr); }           // absolute target — ISO date or datetime
        ...
        const p = c.getAttribute('data-nyx-countdown').split(':'), now = new Date();
  ...
  function initZakat(root) {
    $$('.nyx-zakat', root).filter(z => !z._nyxZ).forEach(z => {
      ...
      const nisab = parseFloat(z.getAttribute('data-nisab')) || 0;   // threshold below which no zakat is due
      ...
      function calc() {
        ...
        const below = nisab > 0 && v < nisab;                       // zakat is owed only at/above nisab
        z.classList.toggle('nyx-below-nisab', below);
        const due = below ? 0 : v * rate;
        if (out) out.textContent = due.toLocaleString(undefined, { maximumFractionDigits: 2 });
      }
  ...
  function initQibla(root) {
    $$('.nyx-qibla[data-nyx-qibla], .nyx-qibla[data-coords]', root).filter(q => !q._nyxQ).forEach(q => {
      ...
      if (coords) { const p = coords.split(',').map(parseFloat); if (p.length === 2 && !isNaN(p[0]) && !isNaN(p[1])) deg = qiblaBearing(p[0], p[1]); }
      if (deg == null) deg = parseFloat(q.getAttribute('data-nyx-qibla')) || 0;
      deg = Math.round(deg * 10) / 10;
      const n = q.querySelector('.needle');if (n) n.style.transform = 'rotate(' + deg + 'deg)';
      if (!q.getAttribute('aria-label')) q.setAttribute('aria-label', 'Qibla direction ' + Math.round(deg) + '° from north');
    });
  }
  ...
  function luhnValid(s) {                                       // Saudi national ID carries a Luhn (mod-10) check digit
    let sum = 0, alt = false;
  ...
  function initIdInput(root) {
    $$('.nyx-id-input', root).filter(w => !w._nyxId).forEach(w => {
      ...
      inp.setAttribute('dir', 'ltr'); inp.setAttribute('inputmode', 'numeric');   // ID digits stay LTR even on RTL pages
      ...
      function check(commit) {
        ...
        const ok = /^[12]\d{9}$/.test(d) && luhnValid(d);           // format + checksum (not format alone)
        const bad = !ok && d.length > 0 && (commit || d.length >= 10);
        w.classList.toggle('is-valid', ok);
        w.classList.toggle('is-invalid', bad);
        inp.setAttribute('aria-invalid', bad ? 'true' : 'false');
      }
      inp.addEventListener('input', () => { check(false); });
      inp.addEventListener('blur', () => { check(true); });
      check(false);
    });
  }

  /* ---------- Hijri ⇄ Gregorian — Umm al-Qura via Intl (zero-data, official),
     with a tabular (Kuwaiti) fallback when the islamic-umalqura calendar is absent. ---------- */
  const HIJRI_MONTHS = ['محرّم', 'صفر', 'ربيع الأول', 'ربيع الآخر', 'جمادى الأولى', 'جمادى الآخرة', 'رجب', 'شعبان', 'رمضان', 'شوّال', 'ذو القعدة', 'ذو الحجّة'];
  const _HEPOCH = 1948439.5;
  function _gToJD(y, m, d) { const a = Math.floor((14 - m) / 12), yy = y + 4800 - a, mm = m + 12 * a - 3; return d + Math.floor((153 * mm + 2) / 5) + 365 * yy + Math.floor(yy / 4) - Math.floor(yy / 100) + Math.floor(yy / 400) - 32045; }
  function _jdToG(jd) { const a = jd + 32044, b = Math.floor((4 * a + 3) / 146097), c = a - Math.floor(146097 * b / 4), e2 = Math.floor((4 * c + 3) / 1461), e = c - Math.floor(1461 * e2 / 4), m = Math.floor((5 * e + 2) / 153); return { y: 100 * b + e2 - 4800 + Math.floor(m / 10), m: m + 3 - 12 * Math.floor(m / 10), d: e - Math.floor((153 * m + 2) / 5) + 1 }; }
  function _isToJD(y, m, d) { return d + Math.ceil(29.5 * (m - 1)) + (y - 1) * 354 + Math.floor((3 + 11 * y) / 30) + _HEPOCH - 1; }
  function _jdToIs(jd) { jd = Math.floor(jd) + 0.5; const y = Math.floor((30 * (jd - _HEPOCH) + 10646) / 10631); const m = Math.min(12, Math.ceil((jd - (29 + _isToJD(y, 1, 1))) / 29.5) + 1); return { y, m, d: (jd - _isToJD(y, m, 1)) + 1 }; }
  function _intlHijri(date) {
    try {
      ...
      const o = {}; for (let i = 0; i < p.length; i++) if (p[i].type !== 'literal') o[p[i].type] = +p[i].value;
      return (o.year && o.month && o.day) ? { y: o.year, m: o.month, d: o.day } : null;
    } catch (e) { return null; }
  }
  /* Gregorian → Hijri. Pass a Date, or (year, month 1-12, day). Returns {y,m,d,month}. */
  ...
  /* Hijri → Gregorian Date (UTC). Tabular seed, refined to official Umm al-Qura when Intl is present. */
  ...
  /* "15 رمضان 1447 هـ". input = Date or {y,m,d}. opts.numerals === 'arab' for Arabic-Indic digits. */
  ...
  function initHijri(root) {
    ...
    $$('[data-nyx-hijri]', root).filter(w => !w._nyxHC).forEach(w => {
      ...
      if (hm && !hm.options.length) HIJRI_MONTHS.forEach((n, i) => { const o = doc.createElement('option'); o.value = i + 1; o.textContent = (i + 1) + ' · ' + n; hm.appendChild(o); });
      function showOut(h) { if (out) out.textContent = formatHijri(h, { numerals: 'arab' }); }
      function fromGreg() {
        if (!greg || !greg.value) return;
        const p = greg.value.split('-'), h = toHijri(+p[0], +p[1], +p[2]);
        if (hd) hd.value = h.d; if (hm) hm.value = h.m; if (hy) hy.value = h.y;
        showOut(h);
      }
      function fromHij() {
        if (!hy || !hd || !hm) return;
        const y = +hy.value, m = +hm.value, d = +hd.value; if (!y || !m || !d) return;
      ...
      [hd, hm, hy].forEach(f => { if (f) f.addEventListener('change', fromHij); });
      if (greg && !greg.value) { const t = new Date(); greg.value = t.getFullYear() + '-' + _pad(t.getMonth() + 1) + '-' + _pad(t.getDate()); }
      fromGreg();
    });
  }
  ...
  function initTabs(root) {
    $$('[data-nyx-tabs]', root).filter(g => !g._nyxTabs).forEach(g => {
      ...
      g.addEventListener('keydown', e => {              // roving arrow-key focus (WAI-ARIA)
        const cur = e.target.closest('[data-nyx-tab]'), i = tabs.indexOf(cur); if (i < 0) return;
        let n = -1;
  ...
  function initHierarchy(root) {
    $$('.nyx-hierarchy li', root).forEach(li => {
      ...
      if (node && kids && !node._nyxH) {
        ...
        const car = doc.createElement('span'); car.className = 'caret'; car.textContent = '▸';
  ...
  function hcolSelect(item) {
    ...
    if (!col || !wrap) return;
    $$('.nyx-hitem', col).forEach(x => { x.classList.toggle('active', x === item); });
    const cols = $$('.nyx-hcol', wrap), idx = cols.indexOf(col);
    cols.forEach((c, n) => { if (n > idx) c.hidden = true; });
    const tgt = item.getAttribute('data-nyx-hcol');
    if (tgt) { const t = $(tgt, wrap); if (t) t.hidden = false; }
  }
  ...
  function initPrayerTimes(root) {
    $$('.nyx-prayer-times[data-nyx-prayers]', root).forEach(wrap => {
      ...
      function update() {
        // re-pick the next prayer as time passes (was computed once and went stale)
        const now = new Date();

        const cur = now.getHours() * 60 + now.getMinutes();
        let pick = null;
        items.forEach(p => {
          const t = p.getAttribute('data-time'); if (!t) return;
      ...
      if ('loading' in HTMLImageElement.prototype) {
        ...
        if (window.IntersectionObserver) {
          const io = new IntersectionObserver((es, ob) => {
            es.forEach(e => { if (e.isIntersecting) { img.src = img.src; ob.disconnect(); } });
          });
          io.observe(img);
        }
        img.addEventListener('load', onL);
  ...
  function initNav(root) {
    $$('.nyx-nav .nav-toggle', root).forEach(btn => {
      ...
      btn.addEventListener('click', () => {
        ...
        if (!open && menu) { const a = menu.querySelector('a'); if(a) a.focus(); }
      });
  ...
  function initSliderNav(root) {
    $$('[data-nyx-slider-nav]', root).filter(nav => !nav._nyxSlNav).forEach(nav => {
      nav._nyxSlNav = true;
      const ind = doc.createElement('span'); ind.className = 'nyx-nav-indicator';
      nav.appendChild(ind);
      ...
      items.forEach(it => {
        it.addEventListener('mouseenter', () => { update(it); });
      ...
      nav.addEventListener('change', e => {
        if (e.target && e.target.matches('input[type="radio"]')) {
          ...
          if (label) {
            $$(selector, nav).forEach(lbl => { lbl.classList.remove('active'); });
  ...
  function initTilt(root) {
    if (prefersReducedMotion()) return;
    $$('.nyx-tilt', root).filter(el => !el._nyxTilt).forEach(el => {
      el._nyxTilt = true;
      const strength = parseFloat(el.dataset.nyxTiltStrength) || 15;
  ...
  function initCounter(root) {
    const els = $$('[data-nyx-count]', root).filter(el => !el._nyxCount);
    ...
    const io = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        const target  = parseFloat(el.dataset.nyxCount)  || 0;
        const suffix  = el.dataset.nyxSuffix  || '';
        const prefix  = el.dataset.nyxPrefix  || '';
        const decimals = parseInt(el.dataset.nyxDecimals, 10) || 0;
        const duration = parseInt(el.dataset.nyxDuration, 10) || 1800;
        if (prefersReducedMotion()) { el.textContent = prefix + target.toFixed(decimals) + suffix; return; }
        const start = 0;
        let startTime = null;
        ...
        function step(ts) {
          ...
          const value = start + easeOut(progress) * (target - start);
  ...
  function initTypewriter(root) {
    $$('.nyx-typewriter', root).filter(el => !el._nyxTW).forEach(el => {
      el._nyxTW = true;
      const text  = el.dataset.nyxText || el.textContent.trim();
      const speed = parseInt(el.dataset.nyxSpeed, 10) || 60;
  ...
  doc.addEventListener('contextmenu', e => {
    const host = e.target.closest('[data-nyx-contextmenu]'); if (!host) return;
    const menu = el(host.getAttribute('data-nyx-contextmenu')); if (!menu) return;
    e.preventDefault();
    $$('.nyx-context-menu.open').forEach(m => { m.classList.remove('open'); });
  ...
  return {
    ...
    setTheme, toggleTheme, setDir, toggleDir, setAccent,
    toArabicNumerals, progress,
    toHijri, fromHijri, formatHijri, qiblaBearing, zatcaQR,
    snackbar, confirm: confirmDialog
  };
});

...
(function (root, factory) {
  ...
  /* read optional year bounds (data-min-year / data-max-year) off the host element */
  function calYr(el, base) {
    var mn = el.getAttribute('data-min-year'), mx = el.getAttribute('data-max-year');
    return { min: mn ? +mn : null, max: mx ? +mx : null };
  }
  ...
  function calMonthHtml(y, m, sel, today, yr) {
    var first = new Date(y, m, 1).getDay(), days = new Date(y, m + 1, 0).getDate();
    var baseY = (today && today.y) || y;
    var lo = (yr && yr.min != null) ? yr.min : baseY - 100;
    var hi = (yr && yr.max != null) ? yr.max : baseY + 10;
  ...
  function initDatepicker(root) {
    $$('[data-nyx-datepicker]', root).filter(function (d) { return !d._nyxDp; }).forEach(function (dp) {
      ...
      var yr = calYr(dp, st.today.y);
      function render() { cal.innerHTML = calMonthHtml(st.y, st.m, st.sel, st.today, yr); }
      render();

...
```js
import 'nyx-css/nyx.css';   // or 'nyx-css/components/tokens.css' + the modules you use
...
| `examples/` | Full-page examples — auth, checkout, dashboard, pricing, Arabic article. |
| `fonts/` | Self-hosted Thmanyah Arabic typeface + `thmanyah.css`. |
| `docs/docs.html` · `docs/docs.ar.html` · `docs/docs.js` | Documentation SPA — a hash router renders one component per route from the registry. |

**Adding a doc page** needs no new HTML — just push an object to the `PAGES` array in `docs/docs.js`:
